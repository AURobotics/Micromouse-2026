...
uint32_t ir_sequence[9] = {
    2200, 0,    0,   // Pulse 1: Only Channel 1 is ON
    0,    2200, 0,   // Pulse 2: Only Channel 2 is ON
    0,    0,    2200 // Pulse 3: Only Channel 3 is ON
};
...
/* USER CODE BEGIN 0 */
char dma_ready = 0;
int32_t total_count_enc_tim3 = 0;
uint16_t last_raw_R = 0;
/* USER CODE END 0 */
...
int main(void)
{
  ...
  HAL_TIM_DMABurst_WriteStart(&htim5, TIM_DMABASE_CCR1, TIM_DMA_CC4,
...
void motor_speed(int16_t duty) // [-100,100]
{
  duty *= 9.99;
    ...
    if (duty >= 0)
    {
        __HAL_TIM_SET_COMPARE(&htim1, MOTOR_RIGHT_FORWARD_CHANNEL, duty);
        __HAL_TIM_SET_COMPARE(&htim1, MOTOR_RIGHT_BACKWARD_CHANNEL, 0);
    }
    else
    {
        __HAL_TIM_SET_COMPARE(&htim1, MOTOR_RIGHT_FORWARD_CHANNEL, 0);
        __HAL_TIM_SET_COMPARE(&htim1, MOTOR_RIGHT_BACKWARD_CHANNEL, -duty);
    }
}
